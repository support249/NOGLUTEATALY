...
- Booking widgets
- Image/media library
- Contact form backend (Brevo wired; needs env keys on host)
- Newsletter / payment badges if still needed

## Known issues

- Local resource folder has no photos; pages are text-first
- Contact form submits via Brevo (`POST /api/contact`); requires `BREVO_API_KEY` + verified `BREVO_SENDER_EMAIL`
